...
//                                                  //  на который указывает первый параметр (указатель), 
     ...
     bzero(buffer, 256);
     int n = read(client_socket, buffer, 255); // getting data from client_socket, writing it in buffer, max_amount of data is 255 bytes
     ...
     n = write(client_socket, "I got your message", 18); // write to the first parameter, from second parameter, amount of bytes - third parameter
